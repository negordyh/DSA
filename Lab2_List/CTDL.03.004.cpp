...
int L1List<T>::insert(int i, T &a)
{
 ...
 if (i > (int)this->_size) return -1;
        ...
        L1Item<T>* _pre = this->_pHead;
        L1Item<T>* _loch = this->_pHead->pNext;
        for (int j = 0; j < i-1; j++) {
            _pre = _pre->pNext;
            _loch = _loch->pNext;
        }
        L1Item<T>* pNew = new L1Item<T>(a);
        if(_pre->pNext!=NULL) pNew->pNext = _loch;
        _pre->pNext=pNew;
    }
    _size++;
    return 0;
...
int L1List<T>::remove(int i)
{
    // TODO
 L1Item<T>* _pre = this->_pHead;
    if (_size == 0 || i >=(int) this->_size) return -1;
    else if (i == 0) {
        this->_pHead = this->_pHead->pNext;
    }
    else {
        for (int j = 0; j < i - 1; j++)_pre = _pre->pNext;
        _pre->pNext = _pre->pNext->pNext;
    }
    _size--;
    return 0;
}
...
void L1List<T>::reverse() {
  ...
  if (_size == 0 || _size == 1) return;
        ...
        p = p->pNext;
        while (p->pNext != NULL) {
            L1Item<T>* tmp=new L1Item<T>(p->data);
            tmp->pNext = pNew;
            pNew = tmp;
            p = p->pNext;
        } 
