...
Node* tmp = this->head;
	if (tmp->coeff <= -1) cout << "-";
	while (tmp != NULL) {
		if (tmp->coeff > 1) cout << tmp->coeff;
		else if (tmp->coeff < -1) cout << -tmp->coeff;
		else if (tmp->coeff == 1 && tmp->exp == 0) cout << 1;
		if (tmp->exp > 0) cout << "x";
		if (tmp->exp > 1)  cout << "^" << tmp->exp;
		if (tmp->link != NULL) {
			if (tmp->link->coeff < 0) cout << " - ";
			else cout << " + ";
		}
		tmp = tmp->link;
